...
func _ready() -> void:
	...
	allRooms.get(startRoom).setTile(generateStartTile())
	print(entranceTile)
...
func placeHolderSlots() -> void: 
	...
	for numY in dimensions.y:
		...
		for numX in dimensions.x:
			var tile: GridTile = mapTileSceen.instantiate()
...
func placePregeneratedTiles(tile: Tile_Data, pos: Vector2i) -> void:
	var selectedTile = allRooms.get(pos) as GridTile
	tile.currentEntrances =  generateEntrances()
	while( !checkPlacement(pos, tile)):
		tile.currentEntrances = generateEntrances()
	selectedTile.setTile(tile)
	selectedTile.generated_tile = true	
	pass
